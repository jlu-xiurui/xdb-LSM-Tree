...
int MemTable::KeyComparator::operator()(const char* a, const char* b) const {
    Slice as = DecodeLengthPrefixedSlice(a);
    Slice bs = DecodeLengthPrefixedSlice(b);
    return comparator.Compare(as, bs);
}

MemTable::MemTable(const InternalKeyComparator& cmp) 
    : comparator_(cmp),table_(comparator_, &mem_buffer_), refs_(0) {}
...
void MemTable::Put(SequenceNum seq, RecordType type, 
    ...
    char* buf = mem_buffer_.Allocate(total_size);
