...
class BlockReader::Iter : public Iterator {
 public:
    ...
    bool Valid() const override {
        return offset_ < restart_offset_;
    }
    ...
    void Seek(const Slice& key) override {
        ...
        while (lo < hi) {
            ...
            const char* p = ParsedEntry(data_ + restart_offset, 
                    data_ + restart_offset_, &shared, &non_shared, &value_len);
 ...
 private:
    ...
    const char* ParsedEntry(const char* p, const char* limit,
            uint32_t* shared,uint32_t* non_shared,uint32_t* value_len) {
    ...
    uint32_t GetRestartOffset(uint32_t restart_index) const {
        ...
        return DecodeFixed32(data_ + restart_offset_ + restart_index*sizeof(uint32_t));
    }
