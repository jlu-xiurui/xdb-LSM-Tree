...
Status SSTableBuilder::status() const {
    return rep_->status_;
}
...
void SSTableBuilder::Add(const Slice& key, const Slice& value) {
    ...
    if (rep_->num_entries_ != 0) {
        assert(rep_->data_block_option_.comparator
                ->Compare(key, Slice(rep_->last_key_) ) > 0);
    }
    ...
    if (rep_->data_block_over_) {
        ...
        rep_->data_block_option_.comparator->
                FindShortestMiddle(&rep_->last_key_,key);
        std::string block_handele;
        rep_->data_block_handle_.EncodeTo(&block_handele);
        rep_->index_block_builder_.Add(rep_->last_key_, Slice(block_handele));
...
void SSTableBuilder::WriteRawBlock(const Slice& contents,
    ...
    if (ok()) {
        ...
        rep_->status_ = rep_->file_->Append(Slice(tail, KBlockTailSize));
        if (ok()) {
...
Status SSTableBuilder::Finish() {
    ...
    if (ok() && rep_->filter_block_builder_ != nullptr) {
        WriteRawBlock(rep_->filter_block_builder_->Finish(),
                KUnCompress, &filter_block_handle);
    }
    if (ok()) {
    ...
    if (ok()) {
        ...
        // in order to decrease the key's length
            rep_->data_block_option_.comparator->
                    FindShortestBigger(&rep_->last_key_);
            std::string block_handele;
            rep_->data_block_handle_.EncodeTo(&block_handele);
            rep_->index_block_builder_.Add(rep_->last_key_, Slice(block_handele));
            rep_->data_block_over_ = false;
