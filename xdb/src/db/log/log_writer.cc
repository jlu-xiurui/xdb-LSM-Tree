...
namespace log {
    ...
    Status Writer::AddRecord(const Slice& slice) {
        ...
        do {
            ...
            if (block_left < KLogHeadSize) {
                if (block_left > 0) {
                    dest_->Append(Slice("\x00\x00\x00\x00\x00\x00\x00", block_left));
                }
                block_offset_ = 0;
            }
    ...
    Status Writer::WritePhysicalRecord(LogRecodeType type, const char* p, size_t len) {
        ...
        Status s = dest_->Append(Slice(header, KLogHeadSize));
        if (s.ok()) {
            s = dest_->Append(Slice(p, len));
            if(s.ok()) {
                s = dest_->Flush();
            }
        }
