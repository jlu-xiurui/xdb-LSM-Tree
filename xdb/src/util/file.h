...
class SequentialFile {
 public:
    ...
    Status Read(size_t n, Slice* result, char* buffer) {
        while (true) {
...
class RandomReadFile {
 public:
    ...
    virtual Status Read(uint64_t offset, uint64_t n, Slice* result ,char* buffer) const = 0;
};

class MmapRandomReadFile : public RandomReadFile{
 public:
    MmapRandomReadFile(std::string filename, char* mmap_base,
        size_t length, Limiter* limiter)
         :  limiter_(limiter),
    ...
    Status Read(uint64_t offset, uint64_t n, Slice* result ,char* buffer) const override{
        if (offset + n > length_) {
            *result = Slice();
...
class PreadRamdomReadFile : public RandomReadFile{
 public:
    PreadRamdomReadFile(std::string filename,Limiter* limiter,
        int fd) : limiter_(limiter),
                  filename_(std::move(filename)), 
