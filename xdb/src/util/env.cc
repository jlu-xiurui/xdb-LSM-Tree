...
class EnvImpl : public Env {
 public:
    EnvImpl() : background_work_cv_(&background_work_mutex_),
    ...
    Status NewRamdomReadFile(const std::string& filename, RandomReadFile** result) override {
        int fd = ::open(filename.data(), O_RDONLY);
        if (fd < 0) {
            *result = nullptr;
            return SystemError(filename, errno);
        ...
        if (status.ok()) {
            void* mmap_base =
                ::mmap(0, file_size, PROT_READ, MAP_SHARED, fd, 0);
            if (mmap_base == MAP_FAILED) {
                status = SystemError(filename, errno);
            } else {
                *result = new MmapRandomReadFile(filename, static_cast<char*>(mmap_base)
    ...
    Status GetChildren(const std::string& dirname, std::vector<std::string>* filenames) override {
        filenames->clear();
...
class SingletonDefaultEnv {
 public:
    SingletonDefaultEnv() {
        new (storage_) EnvImpl;
    }
    Env* Get() {
        return reinterpret_cast<Env*>(storage_);
    }
 private:
