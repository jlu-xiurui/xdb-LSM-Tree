...
Cache::Handle* LRUCache::Insert(const Slice& key, uint32_t hash, void* value, 
    ...
    LRUHandle* handle = reinterpret_cast<LRUHandle*>(malloc(sizeof(LRUHandle) - 1 + key.size()));
    handle->value = value;
...
class ShardLRUCache : public Cache{
 public:
    ...
    Handle* Insert(const Slice& key, void* value,
            size_t charge, void (*deleter)(const Slice& key, void* value)) override {
        uint32_t hash = Hash(key);
        return shard_[Shard(hash)].Insert(key, hash, value, charge, deleter);
    }
 private:
    uint32_t Hash(const Slice& key) {
