...
namespace xdb {
    ...
    TEST(MemTableTest, InternalKeyCmp) {
        ...
        InternalKey a(0, Slice("abc"), KTypeInsertion);
        InternalKey b(1, Slice("abc"), KTypeInsertion);
        ASSERT_EQ(cmp.Compare(a,b), +1);
        a = InternalKey(0, Slice("abc"), KTypeInsertion);
        b = InternalKey(1, Slice("bcd"), KTypeInsertion);
